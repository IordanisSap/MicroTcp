...
server_microtcp (uint16_t listen_port, const char *file)
{
    ...
    client_addr_len = sizeof(struct sockaddr);
    flag = microtcp_accept(&sock, &client_addr,client_addr_len);
    if (flag == -1) {
...
client_microtcp (const char *serverip, uint16_t server_port, const char *file)
{
    ...
    flag = microtcp_connect(&sock,(struct sockaddr *) &sin, sizeof(struct sockaddr_in));
    if (flag==-1 || sock.state == INVALID) {
    ...
    printf ("Data sent. Terminating...\n");
    flag = microtcp_shutdown(&sock, SHUT_RDWR);
    if (flag == -1 || sock.state != CLOSED){
        perror("microTCP shutdown error");
    }
    free (buffer);
    fclose (fp);
    return 0;
